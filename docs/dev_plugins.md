...
class CustomDORService(DORInterface):
    ...
    # Required DORInterface methods (all async except type):
    def type(self) -> str: ...
    async def search(self, patterns, owner_iid, data_type, data_format, c_hashes) -> List[DataObject]: ...
    async def statistics(self) -> DORStatistics: ...
    async def add(self, content_path, data_type, data_format, owner_iid, ...) -> DataObject: ...
    async def remove(self, obj_id) -> Optional[DataObject]: ...
    async def get_meta(self, obj_id) -> Optional[DataObject]: ...
    async def get_content(self, obj_id, content_path) -> None: ...
    async def get_provenance(self, c_hash) -> Optional[DataObjectProvenance]: ...
    async def grant_access(self, obj_id, user_iid) -> DataObject: ...
    async def revoke_access(self, obj_id, user_iid) -> DataObject: ...
    async def transfer_ownership(self, obj_id, new_owner_iid) -> DataObject: ...
    async def update_tags(self, obj_id, tags) -> DataObject: ...
    async def remove_tags(self, obj_id, keys) -> DataObject: ...
```
...
class CustomRTIService(RTIServiceBase):
    ...
    def perform_submit_single(self, job: Job, proc: Processor) -> None: ...  # sync
    def perform_submit_batch(self, batch: List[Tuple], batch_id: str) -> None: ...  # sync
    async def perform_cancel(self, job_id: str, ...) -> None: ...
    async def perform_purge(self, job_record: DBJobInfo) -> None: ...
    async def perform_job_cleanup(self, job_id: str) -> None: ...
    def resolve_port_mapping(self, job_id: str, runner_details: dict) -> dict: ...  # sync
```
